#!/usr/bin/env python3
"""Thin JSON-in / JSON-out CLI for M10 transactional finalization."""
...
from article_loop.finalization import FinalizationError, finalize_decision
...
def main() -> int:
    parser = argparse.ArgumentParser(description="M10 hash-revalidating transactional finalizer")
    ...
    try:
        ...
        receipt = finalize_decision(Path(root_value).resolve(), run_id, cycle_id=cycle_id)
        sys.stdout.write(json.dumps({"status": "success", "receipt": receipt}, indent=2) + "\n")
        return 0
    except ValueError as exc:
        sys.stderr.write(json.dumps({"status": "error", "error_type": "input_error", "message": str(exc)}) + "\n")
        return 1
    except FinalizationError as exc:
        sys.stderr.write(json.dumps({"status": "error", "error_type": "finalization_error", "message": str(exc)}) + "\n")
        return 2
