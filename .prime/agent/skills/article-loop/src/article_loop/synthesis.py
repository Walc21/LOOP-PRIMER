...
class M7Pipeline:
    ...
    def build(self,synthesis: Mapping[str,Any],champion: str|Path):
        ...
        stage=Path(tempfile.mkdtemp(prefix=candidate_id+"-",dir=self.root/"workspaces"))
        try:
            ...
            _atomic(stage/"manifest.json",_json(manifest)); _readonly(stage); self._existing(stage,candidate_id,synthesis,hashes); self._hit("before_rename"); os.chmod(stage,0o700); os.chmod(stage.parent,stat.S_IMODE(stage.parent.lstat().st_mode)|0o700); os.chmod(published.parent,stat.S_IMODE(published.parent.lstat().st_mode)|0o700); os.rename(stage,published); _readonly(published); _fsync(published.parent); self._hit("after_rename")
    ...
    def record_candidate(self,store: DurableStore,manifest: Mapping[str,Any]):
        ...
        return self._record(store,manifest["run_id"],State.CANDIDATE_BUILT,manifest["content_hash"],{"candidate_id":manifest["candidate_id"],"candidate_hash":manifest["content_hash"]})
    def record_gates(self,store: DurableStore,report: Mapping[str,Any]):
        locator=report.get("report_locator")
        if not isinstance(locator,str): raise SynthesisError("gate report is not persisted")
        path=_contained(self.root,locator,True); _regular(path); raw=path.read_bytes()
        try: persisted=json.loads(raw)
        except json.JSONDecodeError as e: raise SynthesisError("gate report is invalid") from e
        try:
            _validate_schema(self.root,"gate-report.schema.json",persisted)
            from .gates import REQUIRED_GATES
            if tuple(item["gate_id"] for item in persisted["gates"]) != REQUIRED_GATES: raise SynthesisError("gate report has an unknown or reordered gate")
        except SynthesisError:
            raise
        except Exception as e:
            raise SynthesisError("gate report schema is invalid") from e
        body={key:value for key,value in persisted.items() if key not in {"report_hash","report_id","report_locator"}}
        expected_hash=_sha(_json(body))
        if persisted.get("report_hash") != expected_hash or persisted.get("report_id") != "g-"+expected_hash[:32]: raise SynthesisError("gate report hash identity differs")
        expected_locator=(Path("state")/"gates"/str(persisted.get("candidate_id",""))/(expected_hash+".json")).as_posix()
        if persisted.get("report_locator") != expected_locator or locator != expected_locator: raise SynthesisError("gate report locator differs")
        if persisted != dict(report) or persisted.get("run_id")!=report.get("run_id") or persisted.get("cycle_id")!=report.get("cycle_id") or persisted.get("candidate_hash")!=report.get("candidate_hash"):
            raise SynthesisError("gate report identity differs")
        if report.get("overall_pass") is not True or report.get("correctness_math_pass") is not True or not isinstance(report.get("report_hash"),str): raise SynthesisError("failed, inconclusive, or unpersisted gates cannot advance")
        candidate_id=report.get("candidate_id")
        if not isinstance(candidate_id,str) or not candidate_id or "/" in candidate_id or "\\" in candidate_id: raise SynthesisError("gate report candidate identifier is invalid")
        try:
            candidate=self.root/"versions/challengers"/candidate_id
            manifest=json.loads((candidate/"manifest.json").read_bytes())
            _validate_schema(self.root,"candidate-manifest.schema.json",manifest)
        except (OSError,json.JSONDecodeError,SynthesisError) as exc:
            raise SynthesisError("gate report candidate is unavailable") from exc
        if (manifest.get("run_id"),manifest.get("cycle_id"),manifest.get("candidate_id"),manifest.get("content_hash"),tree_hash(candidate)) != (report.get("run_id"),report.get("cycle_id"),candidate_id,report.get("candidate_content_hash"),report.get("candidate_hash")):
            raise SynthesisError("gate report does not belong to the published candidate")
